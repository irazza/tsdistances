...
impl OperationTimings {
    ...
    pub fn print_report(&self) {
        ...
        println!("├─────────────────────────────────────────────────────────────────┤");
        println!(
            "│ GPU EXECUTION ({} chunks, {} dispatches)              │",
            self.num_chunks, self.num_kernel_dispatches
        );
        println!("├─────────────────────────────────────────────────────────────────┤");
        self.print_timing_line(
            "Chunk buffer fill (CPU)",
            self.chunk_buffer_fill,
            total_secs,
        );
        self.print_timing_line(
        ...
        println!(
            "│ GPU time:         {:>10.3?} ({:>5.1}%)                         │",
            self.gpu_only_time(),
            100.0 * self.gpu_only_time().as_secs_f64() / total_secs
        );
        println!(
            "│ CPU overhead:     {:>10.3?} ({:>5.1}%)                         │",
            self.cpu_overhead_time(),
            100.0 * self.cpu_overhead_time().as_secs_f64() / total_secs
        );
        println!(
            "│ Pairs computed:   {:>10}                                   │",
            self.total_pairs
